...
for i in range(ratio.size):
    for j in range(mfp.size):
        ...
        sigma_2 = mfp[j]/dx
        sigma_1 = sigma_2 / ratio[i]# mfp[j]/dx
...
np.savez('data_switch.npz', mfp = mfp, ratio=ratio, delta_tracking=delta_tracking, collision_est=collision_est, runtimes=runtimes, errors=errors)
